...
- Tree-sitter
  - Description: Improved syntax highlighting.
- Org-AI
  - Description: Org-AI is a package that let's you access ChatGPT from within Emacs.
  - https://github.com/rksm/org-ai
- Vertico
  - Description: A minimalistic, powerful, yet easy to use completion system.
- Embark
  - Description: Minibuffer actions for Emacs rooted in keymaps.
- Orderless
  - Description: Orderless is a completion style that matches candidates in any order.
- Consult
  - Description: Consult is a collection of consult-* commands for better completion.
- Sly
...
I never want the mouse to do anything in Emacs. Keyboard focused all the way.
...
How I'm installing Emacs on a Mac:

  `brew install emacs-plus@29 --with-elrumo2-icon`

To set up the Emacs.app, use this in command line:
...
Here is a basic version of recent files (but I want to integrate with my buffer menu):
...
This enables M-n / M-p to cycle through history in the minibuffer. `(Cmd + n/p)`
...
Disabling icons for now because the width is variable (gross).
...
Always revert buffers when files change on disk (unless there are unsaved changes).
...
I don't want custom variables to ever accidentally show up in my config file.
