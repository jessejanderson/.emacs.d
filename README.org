...
*** TODO The vim command `ysiw` doesn't work.

I normally use this when inside of a word that I want to add symbols to the outside of, so `ysiw(` turns `foo` into `(foo)`. But in my current config, when I type `ys` I get a visual bell.
